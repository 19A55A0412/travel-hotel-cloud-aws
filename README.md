# Cloud-Based Travel & Hotel Management System

## MSc Cloud Computing Project – University of East London

This project presents a cloud-based, AI-powered Travel and Hotel Management System designed to improve booking efficiency, enhance customer experience, and provide scalable and secure operations using AWS cloud services. The system leverages serverless architecture and intelligent automation to deliver real-time booking, predictive analytics, and personalized services.

---

## Project Overview
The travel and hospitality industry faces challenges such as manual booking, inefficient operations, and lack of personalization. This solution addresses these problems by implementing a modern cloud architecture using AWS to ensure high availability, security, and scalability.

This system automates booking, manages hotel operations, and provides personalized recommendations using AI and predictive analytics.

---

## Cloud Architecture
The system uses a scalable and secure AWS architecture.

<img width="1051" height="663" alt="image" src="https://github.com/user-attachments/assets/ac728db5-07a8-4ee8-b5a7-da9677405739" />


Key components include:
- Route 53 for DNS and traffic routing
- CloudFront for global content delivery
- Elastic Load Balancer for distributing traffic
- EC2 for application hosting
- S3 for static content and storage
- DynamoDB for fast NoSQL data storage
- Lambda for serverless processing
- CloudWatch for monitoring and logging
- IAM for security and access control

---

## AWS Services Used

### AWS Lambda
Serverless backend to process bookings, payments, and chatbot logic.<img width="1051" height="546" alt="image" src="https://github.com/user-attachments/assets/ee9d0791-5ffa-4abe-9b05-8a5f78c18ed5" />

<img width="1051" height="476" alt="image" src="https://github.com/user-attachments/assets/49795e69-51af-43c5-83bf-eb20c51c6453" />


### Amazon S3
Storage for website content, hotel images, and customer files.
<img width="1051" height="458" alt="image" src="https://github.com/user-attachments/assets/47d83e84-7803-4ad0-bf51-7c97396f1ec7" />



### Amazon DynamoDB
High-performance NoSQL database for bookings and user data.
<img width="1051" height="524" alt="image" src="https://github.com/user-attachments/assets/8f277bf5-e7dc-4da7-b12f-eac1c1de40bf" />

### CloudWatch Monitoring
Tracks application performance and logs.

<img width="1051" height="491" alt="image" src="https://github.com/user-attachments/assets/82b4593b-9eb1-4b2f-9052-5745c0b7dbc4" />

### Route 53
Manages domain and traffic routing.
<img width="1037" height="441" alt="image" src="https://github.com/user-attachments/assets/89c6dbd9-f1c4-4722-b657-71d9c2597b66" />


### CloudFront
Delivers content with low latency.

![CloudFront](screenshots/cloudfront.png)

### EC2
Hosts the web application and services.

<img width="1051" height="536" alt="image" src="https://github.com/user-attachments/assets/9ac06688-0526-4a83-aa41-85534edaeb0b" />


---

## Final Hosted Application

### Home Page
<img width="1051" height="562" alt="image" src="https://github.com/user-attachments/assets/a9f66a75-398a-40aa-86ab-2146bc1bea62" />
<img width="952" height="539" alt="image" src="https://github.com/user-attachments/assets/f8cb5d8a-952b-4b4e-9da9-42eb9db56e24" />


---

## Key Features
- Real-time hotel booking
- AI chatbot for customer support
- Predictive analytics and recommendations
- Dynamic pricing
- Secure authentication and authorization
- GDPR-compliant data handling
- High availability and scalability
- Serverless architecture

---

## Functional Requirements
- Role-based access control (RBAC)
- Secure credential management
- Data encryption in transit and at rest
- Network security with public and private subnets
- Monitoring and audit logging

---

## Non-Functional Requirements
- Scalability to support high user traffic
- Performance optimization
- Reliability and fault tolerance
- Usability and accessibility
- Security and compliance
- Maintainability and automation

---

## Cost Optimization
The solution uses serverless services such as AWS Lambda and DynamoDB to reduce infrastructure cost and improve efficiency.
<img width="952" height="539" alt="image" src="https://github.com/user-attachments/assets/6b04d7ac-eca0-4c5b-a270-3f71016c56f9" />
<img width="905" height="540" alt="image" src="https://github.com/user-attachments/assets/9b791779-9ce2-4e79-9a81-dfbe6ee0c373" />


---

## Future Enhancements
- Machine learning recommendation engine
- Big data analytics dashboard
- Microservices architecture
- Kubernetes containerization
- Integration with payment gateways
- Advanced security and disaster recovery

---

## Technologies
- AWS Cloud
- Serverless Computing
- Cloud Security
- AI and Predictive Analytics
- NoSQL Databases
- DevOps and Monitoring

---

## Author
Resitha Sarvana  
MSc Computing Engineering (Cloud Computing)  
University of East London  

---

## Contact
For collaboration, internship, or project discussions, please connect via LinkedIn.
